...
To avoid **overfitting**, we use the **Train Set** to fit the model. 

```{r}
# The basic line of code for the linear model function. 
cars_regression <-lm(dist ~ speed, trainSet)
```
...
As we used a model with a first grade equation, the parameters that define the line that fits our model are 2, **slope** and **y-Intersection**. 
```{r echo=FALSE}
lm(dist ~ speed, trainSet)
